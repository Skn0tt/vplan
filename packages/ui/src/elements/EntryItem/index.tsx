...
import { WithStyles, withStyles, ListItem, ListItemText, ListItemAvatar, Avatar } from 'material-ui';
...
const hash = (input: string): number => _.sum(input.split("").map(c => c.charCodeAt(0)));
const colors = [
  "deepOrange500",
  "blue500",
  "blue900",
  ...
  return (
    ...
    >
      <ListItem className={isMarked ? classes.marked : null}>
        <ListItemAvatar>
          <Avatar className={isMarked ? classes.markedAvatar : classes[color(entry.type)]}>
            {time(entry)}
          </Avatar>
        </ListItemAvatar>
        <ListItemText
