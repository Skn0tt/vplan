...
const parseFile = (mapToEntry: MapToEntry, group: GroupEntries): Parse<Grouped<Entry>> => input => {
  const $ = cheerio.load(input);

  const date = toDate($(".mon_title").text().split(" ")[0], "01:00");
...
export const parseDayInfo = (input: string): DayInfo => {
  ...
  const blockedRoomsString: string = _.find(rowDict, (v, k) => k.startsWith("Blockiert"))
